...
\begin{tikzpicture}[
  ...
  \node[inner sep=0pt,left of=encoder-1-1,xshift=-0.5cm,yshift=-0.8cm] (input) {\includegraphics[width=.25\textwidth]{tikz/spectrogram}};
  ...
  \node [label={above right:$\bs{\mu}^a_{\bs{\mylz}}(\bs{\myos}_n)$}, fit=(mu-1) (mu-3), draw] (mu) {};
  \node [label={below right:$\bs{\sigma}^a_{\bs{\mylz}}(\bs{\myos}_n)$}, fit=(sigma-1) (sigma-3), draw] (sigma) {};
